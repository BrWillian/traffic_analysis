...
void BrandCls::preprocessImage(const cv::Mat &img, float *imgBufferArray) {
...
std::vector<Yolo::Detection> BrandCls::doInference(cv::Mat &img) {
    result.clear();
...
void BrandCls::createContextExecution() {
    ...
    this->engine = static_cast<std::unique_ptr<nvinfer1::ICudaEngine, TRTDelete>>(std::move(runtime->deserializeCudaEngine(brand_engine, brand_engine_len)));
